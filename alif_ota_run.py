...
rootdir="alif_ota_example/"+ASR+"_alpha"+alpha+"_beta"+mel[1]+"_gamma"+mel[0]+"_eta"+eta
savedir=command.replace(" ", "_").replace(".", "").replace("!", "").replace("?", "").replace("'", "").lower()
cline="python alif_ota.py --text "+command+" --rootdir " +rootdir+ " --savedir "+savedir+" --ASR "+ASR+" --gamma "+mel[0]+" --beta "+mel[1]+" --alpha "+alpha+" --eta "+eta
print(cline)
os.system(cline)